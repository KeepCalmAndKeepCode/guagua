...
<template>
	<view id="app" class="container">
		<view class="custom">
			<label class="lab">客户证件号</label>
			<input type="text" value="" placeholder="填写证件号码" class="inp" />
		</view>
		<view class="custom">
			<label class="lab">客户名称</label>
			<input type="text" value="" placeholder="填写客户名称" class="inp" />
		</view>
		<view class="custom lb">
			<label class="lab">客户类别</label>
			<input type="text" value="" placeholder="选择客户类型" id="click_down" class="inp xl" @click="togglePopup('bottom')" />
		...
		<button type="primary" class="btn btnfirst" @click="bc">保存,下一步</button>
